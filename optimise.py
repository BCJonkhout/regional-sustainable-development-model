...
def storage_costs(overproduction: float):
    import main
    storage_cost = 0 # thousands of euros

    while main.statistics(storage_cost, overproduction)[3] < 99.98:
        storage_cost = storage_cost + 500
...
def Solar_Wind_Storage():
    ...
    for i in range(len(overproductions)):
        results = storage_costs(overproductions[i])
    ...
    wss.set_ylabel("CAPEX for Aadorp case, in thousand €")
